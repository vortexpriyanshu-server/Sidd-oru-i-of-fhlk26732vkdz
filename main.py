...
def _button_style(text: str, callback_data: str = "") -> str:
    # Telegram native button styles: primary (blue), success (green), danger (red).
    # Keep neutral/unknown actions primary; only clearly positive/destructive actions
    # receive the corresponding native style.
    value = f"{text} {callback_data}".lower()
    danger_terms = (
        "delete", "remove", "cancel", "stop", "reject", "disable",
        ":ban", "deduct", "bulk_del", "protected_remove", ":del"
    )
    success_terms = (
        "start", "buy", "confirm", "next", "continue", "approve",
        "enable", "activate", "submit"
    )
    if any(term in value for term in danger_terms):
        return "primary"
...
def btn(text: str, callback_data: str, emoji_id: str = None, fallback_emoji: str = "") -> InlineKeyboardButton:
    ...
    style = _button_style(text, callback_data)
    if emoji_id:
        return InlineKeyboardButton(
            text=label, callback_data=callback_data,
            icon_custom_emoji_id=emoji_id, style=style
        )
...
def btn_url(text: str, url: str, emoji_id: str = None, fallback_emoji: str = "") -> InlineKeyboardButton:
    label = f"{fallback_emoji} {sc(text)}".strip() if (fallback_emoji and not emoji_id) else sc(text)
    style = _button_style(text, url)
    if emoji_id:
        return InlineKeyboardButton(
            text=label, url=url,
            icon_custom_emoji_id=emoji_id, style=style
        )
    return InlineKeyboardButton(text=label, url=url, style=style)

def style_btn(text: str, style: str = "primary", request_contact: bool = False, request_location: bool = False) -> KeyboardButton:
    # Reply Keyboard buttons do not support the inline-button "style" field.
...
BOT_TOKEN = "8668180369:AAHD0Ed-XZgW3d0dPGQUtrAz2tzMlcyMYuw"
LOG_CHANNEL_ID = -1003826631526
_DATA_FILE = "blast_data.json"
_PREMIUM_CREDIT_FILE = "PRIYANSHU.json"
...
class S(StatesGroup):
    ...
    premium_sell_uid = State()
    premium_sell_amount = State()
...
def load_premium_credits() -> dict | None:
    """Load paid/premium credits from the dedicated JSON database.

    Invalid/malformed data is rejected safely instead of being used.
    """
    if not os.path.exists(_PREMIUM_CREDIT_FILE):
        return {}
    try:
        with open(_PREMIUM_CREDIT_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        if not isinstance(data, dict):
            log.error("Premium credit DB rejected: root is not an object")
            return None
        cleaned = {}
        for uid, credits in data.items():
            ...
            try:
                ...
                log.error("Premium credit DB rejected invalid user id: %r", uid)
                return None
            if parsed_uid <= 0 or not isinstance(credits, int) or isinstance(credits, bool) or credits < 0:
                log.error("Premium credit DB rejected invalid entry for user %r", uid)
                return None
            cleaned[str(parsed_uid)] = credits
        return cleaned
    except (OSError, json.JSONDecodeError, TypeError, ValueError) as e:
        log.error("Premium credit DB load failed safely: %s", e)
        return None


def save_premium_credits(data: dict) -> bool:
    """Atomically replace the premium credit JSON database."""
    if not isinstance(data, dict):
        return False
    cleaned = {}
    for uid, credits in data.items():
    ...
    tmp_path = _PREMIUM_CREDIT_FILE + ".tmp"
    try:
        with open(tmp_path, "w", encoding="utf-8") as f:
            json.dump(cleaned, f, indent=2, ensure_ascii=False)
            f.flush()
            os.fsync(f.fileno())
        os.replace(tmp_path, _PREMIUM_CREDIT_FILE)
        return True
    except OSError as e:
        log.error("Premium credit DB atomic save failed: %s", e)
        try:
...
def premium_add_credit(uid: int, amount: int) -> tuple[bool, int]:
    if uid <= 0 or amount <= 0:
        return False, 0
    data = load_premium_credits()
    if data is None:
        return False, 0
    new_balance = data.get(str(uid), 0) + amount
    data[str(uid)] = new_balance
    if not save_premium_credits(data):
        return False, 0
    return True, new_balance


def premium_set_credits(data: dict) -> bool:
    return save_premium_credits(data)


def premium_credit_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [btn("SELL CREDIT", "owner:premium:sell", EMOJI_MONEY, "💰")],
        [btn("EXPORT CREDITS", "owner:premium:export", EMOJI_GEAR, "📤")],
        [btn("IMPORT CREDITS", "owner:premium:import", EMOJI_GEAR, "📥")],
...
async def send_random_video(bot: Bot, chat_id: int):
    try:
        videos = db_get_videos()
        if not videos:
            return False

        video_ref = random.choice(videos)
        await bot.send_video(chat_id, video_ref)
        return True
    except Exception as e:
        logger.exception("Failed to send random video: %s", e)
        try:
            await bot.send_message(
                chat_id,
                "⚠️ Video send nahi ho pa raha. Saved video/File ID ya URL check karein."
            )
        except Exception:
            logger.exception("Failed to send video error message")
        return False
...
async def check_membership(bot: Bot, uid: int, channel_id: str) -> bool:
    try:
        ...
        return member.status in ("member", "administrator", "creator") or (member.status == "restricted" and getattr(member, "is_member", False))
    except Exception as e:
...
def owner_kb(d: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [btn("ᴘʀᴇᴍɪᴜᴍ ᴄʀᴇᴅɪᴛs", "owner:premium:menu", EMOJI_CROWN, "👑")],
        ...
        [InlineKeyboardButton(
            text=mode_btn[0],
            callback_data=mode_btn[1],
            style=_button_style(mode_btn[0], mode_btn[1])
        )],
...
@R.callback_query(F.data == "owner:premium:menu")
async def owner_premium_menu(cq: CallbackQuery, state: FSMContext):
    d = load()
    if not is_owner(cq.from_user.id, d):
        await cq.answer("🚫 Owner Only!", show_alert=True)
        return
    await state.clear()
    premium = load_premium_credits()
    if premium is None:
        premium = {}
        premium_status = f"{em(EMOJI_CROSS, '❌')} Invalid database"
    else:
        premium_status = f"{em(EMOJI_CHECK, '✅')} OK"
    total = sum(premium.values())
    await cq.message.edit_text(
        f"{em(EMOJI_CROWN, '👑')} <b>Premium Credit Manager</b>\n\n"
        f"{em(EMOJI_STAR, '👥')} Users: <b>{len(premium)}</b>\n"
        f"{em(EMOJI_MONEY, '💳')} Paid Credits: <b>{total}</b>\n"
        f"DB: {premium_status}",
        reply_markup=premium_credit_kb(),
        parse_mode="HTML"
    )


@R.callback_query(F.data == "owner:premium:sell")
async def owner_premium_sell_start(cq: CallbackQuery, state: FSMContext):
    d = load()
    if not is_owner(cq.from_user.id, d):
        await cq.answer("🚫 Owner Only!", show_alert=True)
        return
    await state.set_state(S.premium_sell_uid)
    await cq.message.edit_text(
        f"{em(EMOJI_MONEY, '💰')} <b>SELL CREDIT</b>\n\n"
        f"Step 1/2: User ka Telegram ID bhejo:",
        reply_markup=kb([(f"{sc('cancel')}", "owner:premium:menu")]),
        parse_mode="HTML"
    )


@R.message(S.premium_sell_uid)
async def owner_premium_sell_uid(msg: Message, state: FSMContext):
    d = load()
    if not is_owner(msg.from_user.id, d):
        await state.clear()
        return
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Valid positive User ID bhejo.", parse_mode="HTML")
        return
    await state.update_data(premium_sell_uid=uid)
    await state.set_state(S.premium_sell_amount)
    await msg.answer(
        f"{em(EMOJI_MONEY, '💳')} <b>Step 2/2</b>\n\nKitne paid credits add karne hain?",
        reply_markup=kb([(f"{sc('cancel')}", "owner:premium:menu")]),
        parse_mode="HTML"
    )


@R.message(S.premium_sell_amount)
async def owner_premium_sell_amount(msg: Message, state: FSMContext):
    d = load()
    if not is_owner(msg.from_user.id, d):
        await state.clear()
        return
    try:
        ...
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Valid positive credit amount bhejo.", parse_mode="HTML")
        return

    fsmd = await state.get_data()
    target_uid = int(fsmd.get("premium_sell_uid", 0))
    old_premium = load_premium_credits()
    if old_premium is None:
        await state.clear()
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Premium database is invalid. SELL CREDIT stopped safely.",
            reply_markup=kb([(f"{sc('back')}", "owner:premium:menu")]),
            parse_mode="HTML"
        )
        return

    premium_new = dict(old_premium)
    premium_balance = premium_new.get(str(target_uid), 0) + amount
    premium_new[str(target_uid)] = premium_balance
    if not premium_set_credits(premium_new):
        await state.clear()
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Premium credit save failed. Existing balances unchanged.",
            reply_markup=kb([(f"{sc('back')}", "owner:premium:menu")]),
            parse_mode="HTML"
        )
        return

    # SELL CREDIT also updates the user's normal usable balance. Roll back the
    # premium file if the existing blast_data.json save fails.
    add_credits(target_uid, amount, d)
    try:
        save(d)
    except Exception as e:
        log.error("SELL CREDIT blast_data save failed; rolling back premium DB: %s", e)
        premium_set_credits(old_premium)
        await state.clear()
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Credit transaction failed safely. Existing balances restored.",
            reply_markup=kb([(f"{sc('back')}", "owner:premium:menu")]),
            parse_mode="HTML"
        )
        return
    new_normal_balance = get_user_credits(target_uid, d)
    await state.clear()
    ...
    try:
        await msg.bot.send_message(
            target_uid,
            f"{em(EMOJI_MONEY, '💰')} <b>Credits Added!</b>\n\n"
            f"+{amount} paid credits mile hain.\n"
            f"{em(EMOJI_MONEY, '💳')} Usable Balance: <b>{new_normal_balance}</b>",
            parse_mode="HTML"
        )
    except Exception:
        pass
    ...
    await msg.answer(
        f"{em(EMOJI_CHECK, '✅')} <b>SELL CREDIT Done</b>\n\n"
        f"User: <code>{target_uid}</code>\n"
        f"Added: <b>{amount}</b>\n"
        f"Normal Balance: <b>{new_normal_balance}</b>\n"
        f"Premium Balance: <b>{premium_balance}</b>",
        reply_markup=kb([(f"{sc('back')}", "owner:premium:menu")]),
        parse_mode="HTML"
    )
...
async def owner_premium_export(cq: CallbackQuery, state: FSMContext):
    ...
    await state.clear()
    if not os.path.exists(_PREMIUM_CREDIT_FILE):
        if not save_premium_credits({}):
            await cq.answer("❌ Premium DB create failed!", show_alert=True)
            return
    try:
        await cq.message.reply_document(
            document=FSInputFile(_PREMIUM_CREDIT_FILE),
            caption=f"{em(EMOJI_CHECK, '✅')} <b>Premium Credits Export</b> — <code>PRIYANSHU.json</code>",
            parse_mode="HTML"
        )
...
async def owner_premium_import_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        f"{em(EMOJI_GEAR, '📥')} <b>IMPORT CREDITS</b>\n\n"
        f"Sirf exact filename <code>PRIYANSHU.json</code> upload karein.\n"
        f"JSON root object hona chahiye: <code>{{\"USER_ID\": CREDIT}}</code>\n\n"
        f"Import existing premium data ko <b>replace</b> karega aur normal credits ko SET karega, ADD nahi.",
        reply_markup=kb([(f"{sc('cancel')}", "owner:premium:menu")]),
        parse_mode="HTML"
    )
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    doc = msg.document
    if doc.file_name != _PREMIUM_CREDIT_FILE:
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Sirf exact <code>PRIYANSHU.json</code> filename accepted hai.",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        cleaned = {}
        for raw_uid, credits in imported.items():
            try:
                uid = int(str(raw_uid))
            except (TypeError, ValueError):
                raise ValueError(f"Invalid user ID: {raw_uid}")
            if uid <= 0:
                raise ValueError(f"Invalid user ID: {raw_uid}")
            if not isinstance(credits, int) or isinstance(credits, bool) or credits < 0:
                raise ValueError(f"Invalid credit value for {uid}")
            cleaned[str(uid)] = credits

        # Replace premium DB atomically first. If this fails, blast_data is untouched.
        old_premium = load_premium_credits()
        if old_premium is None:
            old_premium = {}
        if not premium_set_credits(cleaned):
            raise ValueError("Premium DB save failed")

        # Import is SET/UPDATE, never ADD. Existing imported values are exact balances.
        old_d = deepcopy(d)
        for uid_str, credits in cleaned.items():
            d.setdefault("users", {}).setdefault(uid_str, {})["credits"] = credits
        try:
            save(d)
        except Exception as e:
            log.error("Import blast_data save failed; rolling back premium DB: %s", e)
            premium_set_credits(old_premium)
            raise ValueError("blast_data.json save failed; import rolled back") from e
        await state.clear()

        await msg.answer(
            f"{em(EMOJI_CHECK, '✅')} <b>Import Complete</b>\n\n"
            f"Users: <b>{len(cleaned)}</b>\n"
            f"Premium DB: <code>PRIYANSHU.json</code>\n"
            f"Normal credits: <b>SET/UPDATE</b> (no double credit)",
            reply_markup=kb([(f"{sc('back')}", "owner:premium:menu")]),
            parse_mode="HTML"
        )
    except (UnicodeDecodeError, json.JSONDecodeError, ValueError, TypeError) as e:
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Invalid PRIYANSHU.json: <code>{str(e)[:100]}</code>",
            parse_mode="HTML"
        )
    except Exception as e:
        log.error("Premium import failed: %s", e)
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Import failed safely: <code>{str(e)[:100]}</code>",
            parse_mode="HTML"
        )


@R.message(S.premium_import_file)
async def owner_premium_import_non_file(msg: Message, state: FSMContext):
    d = load()
    if not is_owner(msg.from_user.id, d):
        await state.clear()
        return
    await msg.answer(
        f"{em(EMOJI_WARNING, '⚠️')} Please upload the exact file <code>PRIYANSHU.json</code>.",
        parse_mode="HTML"
    )
...
async def premium_daily_auto_export(bot: Bot):
    """Send the premium credit database daily at 01:01 using local server time."""
    last_export_date = None
    while True:
        now = datetime.now()
        if now.hour == 1 and now.minute == 1 and last_export_date != now.date():
            try:
                if not os.path.exists(_PREMIUM_CREDIT_FILE):
                    save_premium_credits({})
                if os.path.exists(_PREMIUM_CREDIT_FILE):
                    await bot.send_document(
                        MAIN_OWNER,
                        FSInputFile(_PREMIUM_CREDIT_FILE),
                        caption=f"{em(EMOJI_GEAR, '📤')} <b>Daily Premium Export</b> — <code>PRIYANSHU.json</code>",
                        parse_mode="HTML"
                    )
                    last_export_date = now.date()
                    log.info("Daily premium credit export sent")
            except Exception as e:
                log.error("Daily premium export failed: %s", e)
...
async def main():
    ...
    premium_export_task = asyncio.create_task(premium_daily_auto_export(bot))
    log.info("Background scanner task created")
    log.info("Daily premium export task created for 01:01 local server time")
