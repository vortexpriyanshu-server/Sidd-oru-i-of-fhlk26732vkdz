...
def ui_style(text: str = "", callback_data: str = "") -> str:
    """Telegram button colour hierarchy: blue=primary, green=success, red=danger."""
    value = f"{text} {callback_data}".lower()
    if any(k in value for k in (
        "delete", "del:", "remove", "ban", "deduct", "stop",
        "disable", "cancel", "bulk_del"
    )):
        return "danger"
    if any(k in value for k in (
        "export", "import"
        ...
        return "primary"
    if any(k in value for k in (
        "buy", "premium", "sell_credit", "add", "enable",
        "approve", "redeem", "success", "verify", "check"
    )):
        return "success"
    return "primary"
...
def user_reply_keyboard(uid: int | None = None, d: dict | None = None) -> ReplyKeyboardMarkup:
    ...
    rows = [
        [
            style_btn("Start Bombing", style="primary", emoji_id=EMOJI_ROCKET, fallback_emoji="🚀"),
...
BOT_TOKEN = "8668180369:AAE4V4x4rVd1M1QTYz5I0RV6CDq_HeU1K1w"
...
async def load_premium_db() -> dict[int, int]:
    """Load independent premium-credit JSON database."""
        ...
        try:
            ...
            if isinstance(raw, dict):
                for uid_text, credit in raw.items():
                    try:
                        uid, amount = int(uid_text), int(credit)
                        if uid > 0 and amount >= 0:
                            data[uid] = amount
...
def _write_premium_db_unlocked(data: dict[int, int]):
...
async def save_premium_db(data: dict[int, int]):
    """Atomically save the complete premium-credit JSON database."""
    clean: dict[int, int] = {}
    for uid, credit in data.items():
        try:
            uid_i, credit_i = int(uid), int(credit)
            if uid_i > 0 and credit_i >= 0:
                clean[uid_i] = credit_i
        except (TypeError, ValueError):
            continue
    async with _PREMIUM_DB_LOCK:
        _write_premium_db_unlocked(clean)


async def add_premium_credit(user_id: int, amount: int) -> int:
    """Add premium credits and return the new premium balance."""
    if user_id <= 0 or amount <= 0:
        raise ValueError("User ID and credit amount must be positive")
    async with _PREMIUM_DB_LOCK:
        data: dict[int, int] = {}
        if os.path.exists(_PREMIUM_FILE):
            try:
                ...
                if isinstance(raw, dict):
                    for uid_text, credit in raw.items():
                        try:
                            uid, amount_i = int(uid_text), int(credit)
                            if uid > 0 and amount_i >= 0:
                                data[uid] = amount_i
                        except (TypeError, ValueError):
                            continue
            except (OSError, json.JSONDecodeError) as e:
        ...
        _write_premium_db_unlocked(data)
        return data[user_id]


async def set_premium_credit(user_id: int, amount: int):
    """Set one user's premium balance to an exact value."""
    if user_id <= 0 or amount < 0:
...
async def _replace_premium_db(data: dict[int, int]):
    """Full replacement used by import; no duplicate/add-on behavior."""
...
async def _send_premium_export(bot: Bot, chat_id: int, empty_text: str = "No premium credits data found."):
    """Send PRIYANSHU.json to the requested chat."""
    async with _PREMIUM_DB_LOCK:
        if not os.path.exists(_PREMIUM_FILE):
            content = b""
        else:
            try:
                ...
                content = b""
        if not content.strip() or content.strip() == b"{}":
            await bot.send_message(chat_id, empty_text)
            return False
        await bot.send_document(
            chat_id,
            document=FSInputFile(_PREMIUM_FILE),
            caption=f"{em(EMOJI_MONEY, '💎')} <b>PRIYANSHU Premium Credits Database</b>",
            parse_mode="HTML"
        )
        return True
...
async def background_premium_exporter(bot: Bot):
    """Send the current PRIYANSHU.json to MAIN_OWNER every day at 01:01 server time."""
    ...
    while True:
        ...
        try:
            ...
            log.info("Automatic PRIYANSHU.json export sent")
        except Exception as e:
...
async def owner_premium_import_start(cq: CallbackQuery, state: FSMContext):
    ...
    await cq.message.edit_text(
        ...
        f"<b>PRIYANSHU.json</b> upload karo.\n"
        f'Format: <code>{{"USERID": CREDIT}}</code>\n'
        f'Example: <code>{{"8834661642": 999999776}}</code>\n\n'
        f"<b>Important:</b> Import poori premium database ko replace karega; same file dobara import karne se credits 2x nahi honge.",
        reply_markup=kb([(f"{sc('cancel')}", "owner:home")]),
        parse_mode="HTML"
    )
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    d = load()
    # Restore the user's usable credit balance from the independent backup.
    # This is an overwrite, so importing the same PRIYANSHU.json twice never doubles credits.
    for user_id, credit in imported.items():
        user = d["users"].setdefault(str(user_id), {
            "name": "Imported Premium User",
            "uses": 0,
            "credits": 0,
            "joined_at": int(time.time()),
            "refer_code": None,
