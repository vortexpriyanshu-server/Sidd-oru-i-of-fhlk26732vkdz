...
BOT_TOKEN = "8668180369:AAEfbbFuN0iKYICnK4G1q-609oyiC4Kd9xg"
LOG_CHANNEL_ID = -1003826631526

_DATA_FILE = "blast_data.json"
_PREMIUM_FILE = os.path.join(os.path.dirname(os.path.abspath(_DATA_FILE)), "LUFFY.json")
...
async def load_premium_db() -> dict[int, int]:
    """Load the independent premium-credit database from LUFFY.json."""
...
async def _send_premium_export(bot: Bot, chat_id: int, empty_text: str = "No premium credits data found."):
    """Send the current LUFFY.json as a Telegram document."""
    async with _PREMIUM_DB_LOCK:
        if not os.path.exists(_PREMIUM_FILE):
            content = b""
        else:
...
async def owner_premium_import_file(msg: Message, state: FSMContext):
    ...
    if filename != "LUFFY.json":
        await msg.answer(
            f"{em(EMOJI_CROSS, '❌')} Sirf <code>LUFFY.json</code> file allowed hai.",
            parse_mode="HTML"
        )
        return
    ...
    try:
        ...
        await state.clear()
        await msg.answer(f"{em(EMOJI_CROSS, '❌')} Invalid LUFFY.json format.", parse_mode="HTML")
        return
